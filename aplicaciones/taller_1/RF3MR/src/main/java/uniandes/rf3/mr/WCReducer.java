...
public class WCReducer extends Reducer<Text, IntWritable, Text, DoubleWritable> {
    ...
    protected void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
        ...
        for (IntWritable w : values) {
            TripEntity obj = items.stream().filter(x -> x.ZonaHasta == w.get()).findAny().orElse(null);
            if (obj == null) {
                obj = new TripEntity();
                obj.ZonaHasta = w.get();
                obj.Cantidad = 1;
        ...
        context.write(new Text(key + "\tZonaHasta " + max.ZonaHasta + "\tTotalViajes"), new DoubleWritable(max.Cantidad));
    }
}
